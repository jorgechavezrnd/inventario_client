...
void main() {
  group('AuthError', () {
    test('should create user-friendly message with remaining attempts', () {
      ...
      expect(authError.userFriendlyMessage, equals('Usuario o contraseña incorrectos (Intentos: 3)'));
    });

    test('should show last attempt when 0 attempts remaining with status 401', () {
      final responseData = {
        'success': false,
        'message': 'Invalid username or password',
        'errorCode': 'INVALID_CREDENTIALS'
      ...
      expect(authError.userFriendlyMessage, equals('Usuario o contraseña incorrectos (Último intento)'));
    });
    ...
    test('should handle missing headers gracefully', () {
      ...
      expect(authError.message, equals('Usuario o contraseña incorrectos'));
      expect(authError.attemptsRemaining, isNull);
      expect(authError.userFriendlyMessage, equals('Usuario o contraseña incorrectos'));
    });

    test('should create blocked account message with time remaining from headers', () {
      final responseData = {
        'success': false,
      ...
      expect(authError.attemptsRemaining, isNull); // No debe mostrar intentos para cuenta bloqueada
      expect(authError.userFriendlyMessage, contains('Cuenta bloqueada por'));
      expect(authError.userFriendlyMessage, contains('min'));
    });

    test('should create blocked account message with time remaining from JSON lockedUntil', () {
    ...
    test('should handle edge case with 423 status but remaining attempts in headers', () {
      final responseData = {
        'success': false,
      ...
      expect(authError.userFriendlyMessage, equals('Usuario o contraseña incorrectos (Intentos: 2)'));
    });
    ...
    test('should create generic blocked account message without reset time', () {
      final responseData = {
        'success': false,
        'message': 'Account locked due to too many failed attempts',
        'errorCode': 'ACCOUNT_LOCKED'
      ...
      expect(authError.attemptsRemaining, isNull);
      expect(authError.userFriendlyMessage, equals('Cuenta bloqueada temporalmente'));
    });
  });
}